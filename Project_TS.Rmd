...
- mod1: $ARIMA(2,1,0)(1,1,0)_{12}$ process to original log-transformed series: lnserie
- mod2: $ARIMA(0,1,1)(1,1,0)_{12}$  process to original log-transformed series: lnserie
...
From the results, it can be seen that model 2, despite having an higher AIC (but for a little), has a better prediction capability, because focusing on the error metric, it has less predictive error.
...
### Realize an automatic detection of outliers based on the previously fitted $ARIMA(0,1,1)(1,1,0)_{12}$
...
It seems to be cases of presence of the three different types of outliers
...
```{r}
(mod3=arima(lnserie,order=c(2,1,0), seasonal=list(order=c(0,1,1),period=12)))
abs(mod3$coef/sqrt(diag(mod3$var.coef)))
```


```{r}
validation(mod3,dades)
```
...
```{r}
(mod4=arima(lnserie,order=c(0,1,1), seasonal=list(order=c(0,1,1),period=12)))
abs(mod4$coef/sqrt(diag(mod4$var.coef)))
```


```{r}
validation(mod4,dades)
```
...
From the results, it can be seen that model 4 has a better prediction capability, because focusing on the error metric, it has less predictive error, and also it has one of the lowest AIC, and additionally the model has the smallest confidence bounds lenght.
