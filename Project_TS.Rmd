...
-   mod1: $ARIMA(2,1,0)(1,1,0)_{12}$ fitted with the original log-transformed series, which in its explicit statistical expression is  $(1 - B)(1 - B^{12})(1 + 0.7024B + 0.3327B^2) log(X_t) = (1 - 0.4268B^{12}) Z_t$, and has an AIC of -867.91
-   mod2: $ARIMA(0,1,1)(1,1,0)_{12}$ fitted with the original log-transformed series, which in its explicit statistical expression is $(1 - B)^1(1 - B^{12})^1 log(X_t) = (1 - 0.6626B)(1 - 0.4293B^{12}) Z_t$, and has an AIC of -865.81
...
ts.plot(serie,tl1,tu1,pr1,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=c(ultim[1]-2,ultim[1]+3),type="o",main="Model ARIMA(0,1,1)(1,1,0)12")
abline(v=(ultim[1]-2):(ultim[1]+3),lty=3,col=4)

(previs1=window(cbind(tl1,pr1,tu1),start=ultim+c(1,0)))
```

# 5. Calendar effects & Outlier Treatment 
...
```{r}
(modEC=arima(lnserie,order=c(0,1,1), seasonal=list(order=c(1,1,0),period=12),xreg=data.frame(wTradDays,wEast)))
...
mod.atip=outdetec(modEC,dif=c(1,12),crit=2.75,LS=T) # automatic detection of outliers with crit=2.8 and LS =TRUE
...
A small research was conducted to try to interpret some of the values detected as outliers, and the result was not satisfying, because no specific major event that stands out in public records for it corresponding dates. Only one possible phenomenon was found: The observations 7, 6, 12 and 8 occur on the summer (June, July and August) of different years but are additive outliers (AO), whose effect is only noticed at those specific date, and can be directly related to the local people traveling during this season. And this is reflected on the its W_Coeff, which is -0.06 approximately for all this observations, after applying the requires exponential transformation, it is obtained that it is 6% lower than the usual value for that month.
...
(modEC.lin=arima(lnserie.lin,order=c(0,1,1),seasonal=list(order=c(1,1,0),period=12),xreg=data.frame(wTradDays,wEast)))
abs(modEC.lin$coef/sqrt(diag(modEC.lin$var.coef)))
```
...
EfecSS=coef(modEC.lin)["wEast"]*wEast
...
```{r}
d1d12lnserie.lin=diff(diff(lnserieEC.lin,12))
par(mfrow=c(1,2))
...
The feasible models are the same as before:

-   mod3: $ARIMA(2,1,0)(1,1,0)_{12}$
-   mod4: $ARIMA(0,1,1)(1,1,0)_{12}$

## Model 3 (With no outliers and calendar effects)

```{r}
(mod3=arima(lnserieEC.lin,order=c(2,1,0), seasonal=list(order=c(1,1,0),period=12),xreg=data.frame(wTradDays,wEast)))
abs(mod3$coef/sqrt(diag(mod3$var.coef)))
```

It can be seen that the calendar effect terms are not significant,so these are dropped and the model work without them over the new linearized data.

```{r}
(mod3=arima(lnserieEC.lin,order=c(2,1,0), seasonal=list(order=c(1,1,0),period=12)))
abs(mod3$coef/sqrt(diag(mod3$var.coef)))
```
...
ts.plot(serie,tl,tu,pr,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=ultim[1]+c(-3,+2),type="o",main="Model ARIMA(2,1,0)(1,1,0)12+Outliers+CalendarEffects")
abline(v=(ultim[1]-3):(ultim[1]+2),lty=3,col=4)
...
```{r}
(mod4=arima(lnserieEC.lin,order=c(0,1,1), seasonal=list(order=c(1,1,0),period=12),xreg=data.frame(wTradDays,wEast)))
...
```{r}
(mod4=arima(lnserieEC.lin,order=c(0,1,1), seasonal=list(order=c(1,0,0),period=12)))
abs(mod4$coef/sqrt(diag(mod4$var.coef)))
```
...
ts.plot(serie,tl,tu,pr,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=ultim[1]+c(-3,+2),type="o",main="Model ARIMA(0,1,1)(1,1,0)12+Outliers+CalendarEffects")
abline(v=(ultim[1]-3):(ultim[1]+2),lty=3,col=4)
...
row.names(results)=c("ARIMA(0,1,1)(1,1,0)12", "ARIMA(2,1,0)(1,1,0)12+Outliers+CalendarEffects",  "ARIMA(0,1,1)(1,1,0)12+Outliers+CalendarEffects")
results
```

From the results, it can be seen that model 4 is now the best model, despite its prediction capability is slightly worse than the one of model 3, model 4 is simpler because it only has two parameters, so it is the most parsimonious model of these.


## Final Prediction

## Perform long term predictions with the best model (mod4)
...
```{r}
ts.plot(serie,tl1,tu1,pr1,tl2,tu2,pr2,lty=c(1,2,2,1,2,2,1),col=c(1,4,4,2,3,3,6),xlim=ultim[1]+c(1,3),type="o",main="Metro Final Prediction")
legend("topleft",c("ARIMA(0,1,1)(1,1,0)12","ARIMA(0,1,1)(1,1,0)12+CE+Out"),col=c(2,6),lty=1,lwd=2)
